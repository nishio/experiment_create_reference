...
def split_japanese_text_into_sentences(text):
    """
    日本語テキストを文単位に分割する
    ...
    while i < len(fragments):
        if i + 1 < len(fragments) and re.match(pattern, fragments[i+1]):
            sentence = fragments[i] + fragments[i+1]
            sentences.append(sentence)
            i += 2
        elif i < len(fragments):
            if fragments[i].strip():
...
def load_y_data_and_split_into_sentences(file_path):
    ...
    for block in cluster_blocks:
        ...
        if len(lines) > 1 and re.match(r'^\d+件$', lines[1].strip()):
            description = '\n'.join(lines[2:]) if len(lines) > 2 else ""
        else:
            description = '\n'.join(lines[1:]) if len(lines) > 1 else ""
        
        if description:
            sentences = split_japanese_text_into_sentences(description)
    ...
    print(f"Split Y data into {len(all_sentences)} sentences.")
